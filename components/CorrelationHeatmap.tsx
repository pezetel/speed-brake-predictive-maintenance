  ...
  const matrix = useMemo(() => {
    ...
    for (let i = 0; i < fields.length; i++) {
      ...
      for (let j = 0; j < fields.length; j++) {
        const paired = data
          .map(d => ({
            x: d[fields[i] as keyof FlightRecord] as number,
            y: d[fields[j] as keyof FlightRecord] as number,
          }))
      ...
      for (let j = i + 1; j < matrix[i].length; j++) {
        ...
        if (Math.abs(cell.value) > 0.5) {
          ...
          if (cell.xKey.includes('pfd') && cell.yKey.includes('landing')) {
            interpretation = cell.value < 0 ? 'PFD düştükçe iniş mesafesi uzar → Speedbrake etkinliği azalıyor' : 'PFD ile iniş mesafesi pozitif ilişkili';
          } else if (cell.xKey.includes('Deg') && cell.yKey.includes('Deg')) {
            interpretation = 'Açı değerleri birbiriyle güçlü ilişkili → Tutarlı mekanik davranış';
    ...
    <div className={`card ${fullSize ? '' : ''}`}>
          ...
          {matrix.map((row, i) => (
            <div key={i} className="flex items-center">
              ...
              {row.map((cell, j) => (
                ...
                >
                  <span className={`${fullSize ? 'text-xs' : 'text-[9px]'} font-mono`}>
                    {i === j ? '1.00' : cell.value.toFixed(2)}
                  </span>
                </div>
              ))}
            </div>
      ...
      <div className="flex items-center justify-center gap-4 mt-4 text-[10px] text-slate-400">
        <div className="flex items-center gap-1">
          <div className="w-3 h-3 rounded bg-red-600" /> Negatif (güçlü)
        </div>
        <div className="flex items-center gap-1">
          <div className="w-3 h-3 rounded bg-slate-600" /> Zayıf / Yok
        </div>
        <div className="flex items-center gap-1">
          <div className="w-3 h-3 rounded bg-blue-600" /> Pozitif (güçlü)
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {keyCorrelations.map((kc, i) => (
              <div key={i} className={`rounded-lg p-2.5 border text-xs ${
                kc.value > 0 ? 'bg-blue-500/5 border-blue-500/20' : 'bg-red-500/5 border-red-500/20'
              }`}>
                <div className="flex items-center justify-between">
                  <span className="font-medium text-slate-300">{kc.pair}</span>
                  <span className={`font-mono font-bold ${kc.value > 0 ? 'text-blue-400' : 'text-red-400'}`}>
                    r={kc.value.toFixed(3)}
                  </span>
                </div>
